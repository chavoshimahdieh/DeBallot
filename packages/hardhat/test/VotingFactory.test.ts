...
describe("VotingFactory", () => {
  ...
  describe("#create{Voting, Registration}", () => {
    ...
    it("should revert if trying to create a voting or registration with non-existing type", async () => {
      await expect(votingFactory.createVoting("Non-existing", votingParamsEncoded)).to.be.revertedWithCustomError(
        votingFactory,
        "PoolTypeDoesNotExist()",
      );

      await expect(votingFactory.createRegistration("Non-existing", votingParamsEncoded)).to.be.revertedWithCustomError(
        votingFactory,
        "PoolTypeDoesNotExist()",
      );
      ...
      ).to.be.revertedWithCustomError(votingFactory, "PoolTypeDoesNotExist()");
    ...
    it("should revert if trying to create a voting that does not support IVotingPool interface", async () => {
      await expect(
        votingFactory.createVoting(REGISTRATION_TYPE, registrationParamsEncoded),
      ).to.be.revertedWithCustomError(votingFactory, "VotingPoolDoesNotSupportIVotingPool()");
  ...
  describe("#upgradeImpleemntation", () => {
    it("should upgrade the implementation only by the owner", async () => {
      ...
      await expect(
        votingFactory.connect(FIRST).upgradeTo(await newImplementation.getAddress()),
      ).to.be.revertedWithCustomError(votingFactory, "OnlyRegistryOwnerCanUpgrade()");
